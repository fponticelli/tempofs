...
import { append, map, empty, filter, length, cons, reverse, collect, iterate, singleton } from "./.fable/fable-library.3.1.10/List.js";
import { some } from "./.fable/fable-library.3.1.10/Option.js";
import { printf, toConsole, interpolate, toText } from "./.fable/fable-library.3.1.10/String.js";
import { remove } from "./HtmlTools.fs.js";
import { partialApply, mapCurriedArgs, curry, equals } from "./.fable/fable-library.3.1.10/Util.js";
...
export class HTMLGroupImpl {
    ...
    Append(child) {
        ...
        if (child["GetNodes"] !== null) {
            ...
            toConsole(interpolate("Appending to Group %P() nodes", [length(nodes)]));
            iterate((node) => {
    ...
    Remove(child) {
        ...
        if (child["GetNodes"] !== null) {
            ...
            console.log(some(toText(interpolate("Before %P()", [length(this$.children)]))));
            this$.children = filter((c) => (!equals(c, child)), this$.children);
            console.log(some(toText(interpolate("After %P()", [length(this$.children)]))));
...
export function HTMLTrigger$3$reflection(gen0, gen1, gen2) {
    return class_type("Tempo.Html.HTMLTrigger`3", [gen0, gen1, gen2], HTMLTrigger$3);
}

export function HTMLTrigger$3_$ctor_Z685BAA8A(handler) {
    return new HTMLTrigger$3(handler);
}

export function HTMLTrigger$3__get_Handler(this$) {
...
export function makeTrigger(f) {
    return packHTMLTrigger(HTMLTrigger$3_$ctor_Z685BAA8A(f));
}
        ...
        void unpackHTMLTrigger(domTrigger, {
            Invoke(t) {
                el.addEventListener(name, (e) => {
                    dispatch(HTMLTrigger$3__get_Handler(t)(state())(e));
                });
                return 0;
            },
        });
...
export function MakeHTMLRender$3__MakeRenderDOMText_Z320284C0(this$, value) {
    return (parent) => ((state) => {
        if (value.tag === 0) {
            ...
            toConsole(printf("Text Node Literal"));
            parent.Append(impl_1);
            toConsole(printf("Text Node Literal (Appended)"));
            ...
            const impl = htmlImpl;
            toConsole(interpolate("Text Node Derived: %P()", [htmlImpl.text.nodeValue]));
            parent.Append(impl);
            toConsole(printf("Text Node Derived (Appended)"));
