...
module View =
    ...
    let middleware
        ({ Current = { FilterComments = filterComments }
           Previous = prev
           Action = action
           Request = request }: MiddlewarePayload<_, _, _>)
        =
        ...
        El(
            ...
            [ cls "bg-white shadow-sm lg:static lg:overflow-y-visible"
              DIV [ cls "max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"
                    DIV [ cls "relative flex justify-between xl:grid xl:grid-cols-12 lg:gap-8"
                          DIV [ cls "min-w-0 flex-1 md:px-8 lg:px-0 xl:col-span-6"
                                DIV [ cls
    ...
    let template: Template<Html2TempoState, Html2TempoAction, Html2TempoQuery> =
        DIV [ cls "flex flex-col h-screen"
              bar
              DIV [ cls "flex h-full"
                    DIV [ cls "flex-auto h-full w-6/12"
                          MonacoEditor(
                              (fun _ ->
                                  {| value = sample
                                     language = "html"
